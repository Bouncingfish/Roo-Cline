...
interface ChatTextAreaProps {
	...
	readWriteMode: boolean // Add this prop
	setReadWriteMode: (value: boolean) => void // Add this prop
}

const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(function ChatTextArea(
	{
		inputValue,
		setInputValue,
		textAreaDisabled,
		...
		mode,
		setMode,
		readWriteMode, // Add this prop
		setReadWriteMode, // Add this prop
	},
	ref,
) {
			...
			<div
				...
				<div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
					...
					<span
						...
						style={{ fontSize: 15 }}
					/>
					{/* Edit/Eye toggle button */}
					<span
						className={`input-icon-button ${textAreaDisabled ? "disabled" : ""} codicon ${
							readWriteMode ? "codicon-eye" : "codicon-edit"
						}`}
						onClick={() => {
							if (!textAreaDisabled) {
								setReadWriteMode(!readWriteMode)
								vscode.postMessage({
									type: "mode",
									text: (!readWriteMode ? 1 : 0).toString(),
								})
							}
						}}
						style={{
							fontSize: 15,
							color: readWriteMode ? "var(--vscode-input-foreground)" : "#ffa500",
						}}
					/>
				</div>
			</div>
