...
3. Get access to the API with the following code:
    ...
    ```ts
    const clineExtension = vscode.extensions.getExtension<ClineAPI>("synthience.synthience-cline")
    ...
    **Note:** To ensure that the `synthience.synthience-cline` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

    ```json
    "extensionDependencies": [
        "synthience.synthience-cline"
    ]
    ```
