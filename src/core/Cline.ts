...
export class Cline {
	...
	async ask(
		...
		if (this.abort) {
			throw new Error("Synthience instance aborted")
		}
		let askTs: number
	...
	async sayAndCreateMissingParamError(toolName: ToolUseName, paramName: string, relPath?: string) {
		await this.say(
			"error",
			`Synthience tried to use ${toolName}${
							...
							if (block.partial) {
								...
								if (mcp_arguments) {
									try {
										...
										await this.say(
											"error",
											`Synthience tried to use ${tool_name} with an invalid JSON argument. Retrying...`,
										)
										pushToolResult(
											formatResponse.toolError(
		...
		if (this.consecutiveMistakeCount >= 3) {
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "Synthience uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.5 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
				userContent.push(
					...[
